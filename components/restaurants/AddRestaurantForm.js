...
export default function AddRestaurantForm({ toastRef, setLoading, navigation }) {
    ...
    const addRestaurant = async() =>{
        ...
        if(!responseAddDocument.statusResponse){
            toastRef.current.show("Error al guardar el restaurante, por favor intenta más tarde.", 3000)
        }
    ...
    const validForm = () => {
        ...
        if(size(formData.phone)< 10){
            setErrorPhone("Debes ingresar un teléfono de restaurante valido.")
            isValid = false
        }
        ...
        if(!locationRestaurant){
            toastRef.current.show("Debes de localizar eñ restaurante en el mapa.", 3000)
            isValid = false
